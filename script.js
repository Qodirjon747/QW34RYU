...
const cardOta=document.getElementById("card__ota")
...
function chizish(data){
   ...
   data.map(mahsulot=>{
      ...
      div.innerHTML=`<img src="./img/image 30.png" alt="sd">
      <h4>${mahsulot.nomi}</h4>
      <h3><i class="fa-brands fa-bitcoin"></i> ${mahsulot.coin} coins</h3>
      <p>${mahsulot.nechta} dona qolgan</p>
      `;
...
input1.addEventListener("input",()=>{
   const a=mahsulotlar.filter(mahsulot=> mahsulot.nomi.toLowerCase().includes(input1.value.toLowerCase()));
   chizish(a)
})
